    ...
    <div className="space-y-6">
      ...
      >
        ...
        {musclesForView.map((muscle) => {
          ...
          return (
            <div key={muscle.id} className="absolute inset-0">
              <button
                type="button"
                aria-label={`Select ${muscle.label}`}
                title={muscle.label}
                disabled={!isInteractive}
                onMouseEnter={isInteractive ? () => setHoveredId(muscle.id) : undefined}
                onMouseLeave={isInteractive ? () => setHoveredId(null) : undefined}
                onFocus={isInteractive ? () => setHoveredId(muscle.id) : undefined}
                onBlur={isInteractive ? () => setHoveredId(null) : undefined}
                onPointerDown={isInteractive ? () => setHoveredId(muscle.id) : undefined}
                onClick={isInteractive ? () => onToggle?.(muscle) : undefined}
                ...
                style={{
                  ...
                  background: isSelected
                    ? 'radial-gradient(circle at 50% 40%, rgba(16,185,129,0.42), rgba(16,185,129,0.32))'
                    : 'radial-gradient(circle at 50% 40%, rgba(52,211,153,0.28), rgba(110,231,183,0.15))',
                  opacity: 0,
                  cursor: isInteractive ? 'pointer' : 'default',
                }}
              >
                <span className="sr-only">
                  {isSelected ? `Remove ${muscle.label}` : `Add ${muscle.label}`}
                </span>
              </button>
