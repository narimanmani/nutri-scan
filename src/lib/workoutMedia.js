...
export function findExerciseAnimation(name) {
  ...
  if (normalizedName.length >= 5) {
    ...
    for (const entry of animationIndex) {
      const entryKey = entry.normalized;
      if (!entryKey) continue;

      const entryContains = entryKey.includes(normalizedName);
      const nameContains = normalizedName.includes(entryKey);
      if (!entryContains && !nameContains) {
        continue;
      }

      const overlapLength = entryContains ? normalizedName.length : entryKey.length;
      const coverageRatio = overlapLength / Math.max(normalizedName.length, entryKey.length);

      if (coverageRatio >= 0.55 && coverageRatio > bestPartialScore) {
        bestPartialScore = coverageRatio;
    ...
    if (bestPartialMatch) {
      return {
        src: bestPartialMatch.src,
        alt: `${bestPartialMatch.baseName} demonstration`,
      };
    }
  }
